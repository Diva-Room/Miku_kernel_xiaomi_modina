...
void mi_dsi_panel_state_count(struct dsi_panel *panel, PANEL_COUNT_EVENT event, int value)
{
	...
	case PANEL_ACTIVE:
	{
		...
		if (value && !panel->mi_count.panel_active_count_enable) {
			...
			pr_info("%s: %s on_times[%llu] off_times[%llu]\n", __func__, HWMON_CONPONENT_NAME, on_times[disp_index], off_times[disp_index]);
	...
	case PANEL_BACKLIGHT:
	{
		...
		if (last_bl_start[disp_index] == 0) {
			last_bl_lvl[disp_index] = value;
			last_bl_start[disp_index] = bl_level_end;
			return;
		}

		if (last_bl_lvl[disp_index] > 0) {
			panel->mi_count.bl_level_integral += last_bl_lvl[disp_index] * (bl_level_end - last_bl_start[disp_index]);
			panel->mi_count.bl_duration += (bl_level_end - last_bl_start[disp_index]);
		}

		/* backlight level 2047*0.75 ==> 1535 */
		if (last_bl_lvl[disp_index] > 1535) {
			panel->mi_count.bl_highlevel_duration += (bl_level_end - last_bl_start[disp_index]);
		} else if (last_bl_lvl[disp_index] > 0) {
	...
	case PANEL_HBM:
	{
		...
		if (value > panel->mi_cfg.hbm_backlight_threshold && enter_flag[disp_index]) {
			time_hbm_on[disp_index] = get_jiffies_64();
			enter_flag[disp_index] = 0;
			exit_flag[disp_index] = 1;
		} else if (value <= panel->mi_cfg.hbm_backlight_threshold && exit_flag[disp_index]) {
			time_hbm_off = get_jiffies_64();
			if (time_after64(time_hbm_off, time_hbm_on[disp_index])) {
				panel->mi_count.hbm_duration += (time_hbm_off-time_hbm_on[disp_index]);
	...
	case PANEL_FPS:
	{
		...
		if (first_into_flag[disp_index]) {
			...
			cur_fps_index[disp_index] = change_fps_index;
			return;
		}

		if (change_fps_index != cur_fps_index[disp_index]) {
			jiffies_time = get_jiffies_64();
			if (cur_fps_index[disp_index] != FPS_MAX_NUM) {
				if (time_after64(jiffies_time, timestamp_fps[disp_index]))
					panel->mi_count.fps_times[cur_fps_index[disp_index]] += (jiffies_time - timestamp_fps[disp_index]);
				snprintf(ch, sizeof(ch), "%llu", panel->mi_count.fps_times[cur_fps_index[disp_index]] / HZ);
				update_hw_monitor_info(HWMON_CONPONENT_NAME, hwmon_key_fps[cur_fps_index[disp_index]], ch);
			}
			timestamp_fps[disp_index] = get_jiffies_64();
		}
		cur_fps_index[disp_index] = change_fps_index;
...
void mi_dsi_panel_HBM_count(struct dsi_panel *panel, int enable, int off)
{
	...
	if (off) {
		if (last_HBM_status[disp_index] == 1)
			record = true;
	} else {
		if (enable) {
			if (!last_HBM_status[disp_index]) {
				/* get HBM on timestamp */
				timestamp_hbmon[disp_index] = get_jiffies_64();
			}
		} else if (last_HBM_status[disp_index]){
			record = true;
		}
	}
	...
	if (record) {
		...
		jiffies_time = get_jiffies_64();
		if (time_after64(jiffies_time, timestamp_hbmon[disp_index]))
			panel->mi_count.hbm_duration += (jiffies_time - timestamp_hbmon[disp_index]) / HZ;
	...
	last_HBM_status[disp_index] = enable;
...
void mi_dsi_panel_HBM_count_bl(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if(bl_lvl > panel->mi_cfg.hbm_backlight_threshold && enter_flag[disp_index]) {
		time_hbm_on[disp_index] = get_jiffies_64();
		enter_flag[disp_index] = 0;
		exit_flag[disp_index] = 1;
	} else if (bl_lvl <= panel->mi_cfg.hbm_backlight_threshold && exit_flag[disp_index]) {
...
void mi_dsi_panel_fps_count(struct dsi_panel *panel, u32 fps, u32 enable)
{
	...
	if (!panel || (!enable && timming_index[disp_index] == FPS_MAX_NUM))
		return;

	if (enable && !fps)
		fps = cur_fps[disp_index];
	...
	if (i < FPS_MAX_NUM || !fps) {
		if (i != timming_index[disp_index] && timming_index[disp_index] < FPS_MAX_NUM) {
			jiffies_time = get_jiffies_64();
			if (time_after64(jiffies_time, timestamp_fps[disp_index]))
				panel->mi_count.fps_times[timming_index[disp_index]] += (jiffies_time - timestamp_fps[disp_index]) / HZ;
			snprintf(ch, sizeof(ch), "%llu", panel->mi_count.fps_times[timming_index[disp_index]]);
			update_hw_monitor_info(HWMON_CONPONENT_NAME, hwmon_key_fps[timming_index[disp_index]], ch);
			timestamp_fps[disp_index] = jiffies_time;
		} else if (timming_index[disp_index] == FPS_MAX_NUM)
			timestamp_fps[disp_index] = get_jiffies_64();

		timming_index[disp_index] = i;
	}

	cur_fps[disp_index] = fps ? fps : cur_fps[disp_index];
}
